...
export const loadClashPicks = createServerFn({ method: "GET" }).handler(
  async () => {
    try {
      ...
      if (!res.ok) return CLASH_FALLBACK;
      const payload = (await res.json()) as Array<{
        result?: { data?: { json?: { events?: ClashEvent[] } } };
      }>;
      const events = payload[0]?.result?.data?.json?.events ?? [];
      const rows: MarketRow[] = [];
      for (const event of events) {
        if (event.closed || event.active === false) continue;
        const title = event.title?.trim() || "Long shot";
        let best = 1;
        let pick = "";
        for (const market of event.markets ?? []) {
          const pct = toNumber(market.yesPercentage);
          if (pct == null) continue;
          const p = pct > 1 ? pct / 100 : pct;
          const name = market.title?.trim() ?? "";
        ...
        const question =
          pick && pick !== title ? `${title} — ${pick}` : title;
        rows.push({
          id: `cp-${event.id ?? slug ?? rows.length}`,
          question,
      ...
      return unique.length >= 3 ? unique : CLASH_FALLBACK;
    } catch {
      return CLASH_FALLBACK;
    }
  },
);
